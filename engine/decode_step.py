...
split-K Triton weight streams. The packed Q/K/V consumer adds and rounds the
FP32 partials; o_proj and down_proj finish the residual add and the next
RMSNorm inside their own launch; gate/up carries SiLU * up in its epilogue.
Larger row counts and tile-misaligned shapes keep F.linear. The rule is static
per shape.
"""
...
from kernels.skinny_gemm import linear_add_rms_norm, linear_partials, linear_silu_mul, supports
...
def _project_add_norm(x, weight, residual, norm):
    """(residual + x @ weight.T, norm of that sum), fused when decode-sized."""
    if supports(x, weight):
        return linear_add_rms_norm(x, weight, residual, norm.weight, norm.variance_epsilon)
    return add_rms_norm(residual, F.linear(x, weight), norm.weight, norm.variance_epsilon)
...
def fused_decode_forward(model, cache, tokens, position, mask, context, cos, sin):
    ...
    for index, layer in enumerate(layers):
        ...
        x, normed = _project_add_norm(
            attended.view(batch, -1), attn.o_proj.weight, x, layer.post_attention_layernorm,
        )
        ...
        x, normed = _project_add_norm(hidden, layer.mlp.down_proj.weight, x, following)
