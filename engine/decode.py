...
def qwen_forward(model, input_ids, cache, positions, attention_mask=None, flash_context=None):
    compact = getattr(model, "compact_decode", None)
    if (compact is not None and flash_context is not None
            and input_ids.shape[1] == 1 and not cache.prefilling):
        return compact.forward(input_ids, cache, positions, attention_mask, flash_context)
    base = model.model
